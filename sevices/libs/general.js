...
$(document).ready(function () {
    ...
    (new notyPromotion('#isPushAdsInvited', 'isPushAdsInvitedShowed', 'noty-push-monetization')).show();
    (new notyPromotion('#notyFacebookBanner', 'isNotyFacebookBannerShowed', 'noty-fb-messengers')).show();
...
function setCursorToTextField(sBlockName) { // Feature #12775
    ...
    if ($(sBlockName + ' input[type=text], ' + sBlockName + ' input[type=email], ' + sBlockName + ' input[type=password], ' + sBlockName + ' input[type=number], ' + sBlockName + ' textarea').length > 0) {
        // ÐµÑÐ»Ð¸ Ð½Ð°Ð¹Ð´ÐµÐ½Ð¾ Ð²Ð¸Ð´Ð¸Ð¼Ð¾Ðµ Ð¿ÑƒÑÑ‚Ð¾Ðµ Ð¿Ð¾Ð»Ðµ, ÑÑ‚Ð°Ð²Ð¸Ð¼ ÐºÑƒÑ€ÑÐ¾Ñ€ Ð² Ð½ÐµÐ³Ð¾
        ...
        if (!bSetFocus) {
            // Ð²Ð¸Ð´Ð¸Ð¼Ð¾Ðµ Ð¿Ð¾Ð»Ðµ ÑÐ¾ Ð·Ð½Ð°Ñ‡ÐµÐ½Ð¸ÐµÐ¼, ÑÑ‚Ð°Ð²Ð¸Ð¼ ÐºÑƒÑ€ÑÐ¾Ñ€ Ð² Ð½ÐµÐ³Ð¾
            $(sBlockName + ' input:visible[type=text], ' + sBlockName + ' input:visible[type=email], ' + sBlockName + ' input:visible[type=password], ' + sBlockName + ' input:visible[type=number]').each(function () {
        ...
        if (!bSetFocus) {
            //Ð¿ÐµÑ€Ð²Ð¾Ðµ Ð¿ÑƒÑÑ‚Ð¾Ðµ Ð¿Ð¾Ð»Ðµ, ÑÑ‚Ð°Ð²Ð¸Ð¼ ÐºÑƒÑ€ÑÐ¾Ñ€ Ð² Ð½ÐµÐ³Ð¾
...
$('#tab-bookmarks li a, div.bookmark-arrow, #search-filter a, .need-set-focus a, .set-focus-after-click').on("click", function () { // Ð¿ÐµÑ€ÐµÑ…Ð¾Ð´ Ð¿Ð¾ Ð²ÐºÐ»Ð°Ð´ÐºÐ°Ð¼, Ñ„Ð¸Ð»ÑŒÑ‚Ñ€
...
function refreshJsPagination() {
    $('#go-to-page').on('click', function () {
        ...
        if (pageNum != 'undefined') {
            ...
            if(document.getElementById('tester_user')) { // #17490; @TODO: Ð¿Ð¾Ñ‚Ð¾Ð¼ ÑƒÐ±Ñ€Ð°Ñ‚ÑŒ!       
...
function validEmail(email) {
    ...
    var filter = /^([\w-]+(?:\.[\w-]+)*)@((?:[A-Za-zÐ-Ð¯Ð°-Ñ0-9-]+\.)*[A-Za-zÐ-Ð¯Ð°-Ñ0-9-]{0,66})\.([A-Za-zÐ-Ð¯Ð°-Ñ0-9-]{2,15}(?:\.[A-Za-zÐ-Ð¯Ð°-Ñ0-9-]{2})?)$/i; // #17705
    return filter.test(email);
}
function validUrl(url) {
    var u = /http(s?):\/\/[A-Za-zÐ-Ð¯Ð°-Ñ0-9_\-\.]{3,}\.[A-Za-zÐ-Ð¯Ð°-Ñ0-9_\-]{2,3}/;
    return u.test(url);
}
...
// QUEST HOOK
        ...
        function merciPayment(service,abbr,monthes) {
    ...
    "Ä°": "i"
        ...
        function check_url(url) {
            /*
             ...
             var u = /^(http|https):\/\/?[-a-zÐ°-Ñ0-9]+([\-\.]{1}[a-zÐ°-Ñ0-9]+)*\.[a-zÐ°-Ñ]{2,5}(:[0-9]{1,5})?(\/.*)?/gi;
             */
            var u = /http(s?):\/\/[A-Za-zÐ-Ð¯Ð°-Ñ0-9_\-\.]{1,}\.[A-Za-zÐ-Ð¯Ð°-Ñ0-9_\-]{2,2}/;
...
function downloadDocInExcelFormat(orderId) {
    ...
    //30 - ÑÑ‡ÐµÑ‚ Ð´Ð»Ñ ÑŽÑ€Ð»Ð¸Ñ†Ð°, 40 - ÐºÐ²Ð¸Ñ‚Ð°Ð½Ñ†Ð¸Ñ Ð² Ð±Ð°Ð½Ðº Ð´Ð»Ñ Ñ„Ð¸Ð·Ð»Ð¸Ñ†Ð°
